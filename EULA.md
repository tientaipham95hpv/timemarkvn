...
### 3. Miễn trừ trách nhiệm về Dữ liệu và Sử dụng địa chỉ thủ công
- **Độ chính xác dữ liệu**: Sai số của tọa độ GPS, la bàn và thời gian phụ thuộc hoàn toàn vào cảm biến phần cứng của thiết bị và điều kiện thời tiết/môi trường xung quanh tại hiện trường. Chúng tôi không chịu trách nhiệm đối với bất kỳ thiệt hại nào phát sinh do sự sai lệch của các số liệu này.
- **Tính năng nhập thủ công**: Ứng dụng cung cấp tính năng nhập tay địa chỉ linh hoạt để phục vụ công tác làm việc trong không gian kín. Người dùng hoàn toàn tự chịu trách nhiệm về tính trung thực của văn bản tự nhập. Nhãn `[Thủ công]` (hoặc `[Manual]`) được in rõ trên ảnh để đảm bảo tính minh bạch.
...
- **Telemetry Data Accuracy**: GPS coordinates, compass headings, and weather data depend on device hardware sensors and local environments. We are not liable for any business errors or damages caused by minor sensor inaccuracies.
- **Manual Input Transparency**: The app provides a custom address input feature for indoor/flexible workspaces. Users are solely responsible for the authenticity of manually entered texts. The `[Manual]` tag is embedded transparently on the final image to prevent misrepresentation.
